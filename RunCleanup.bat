 ...
 "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; ^
  try {
      Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/PinkyCodeMaster/SchoolLaptopCleanup/refs/heads/main/SchoolLaptopCleanup.ps1' `
        -OutFile (Join-Path $env:TEMP 'SchoolLaptopCleanup.ps1') -ErrorAction Stop
      exit 0
  }
  catch {
      Write-Output 'Download failed, using local script if available...'
      exit 1
  }"
...
if exist "%TEMP%\SchoolLaptopCleanup.ps1" (
    ...
    start "" powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%TEMP%\SchoolLaptopCleanup.ps1"
) else if exist "%~dp0SchoolLaptopCleanup.ps1" (
    echo Running local script from same folder...
    start "" powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%~dp0SchoolLaptopCleanup.ps1"
) else (
